...
streams *get_stream(int i) {
    return (i >= 0 && i < MAX_STREAMS && st[i] && st[i]->enabled) ? st[i]
                                                                  : NULL;
}
...
int start_play(streams *sid, sockets *s) {
    ...
    if (sid->type == 0) {
        //			LOG("Assuming RTSP over TCP for stream %d, most
        // likely
        // transport was not specified", sid->sid); sid->type
        // =
        // STREAM_RTSP_TCP; 			sid->rsock = s->sock;
        LOG_AND_RETURN(-454,
                       "No Transport header was specified, for stream sid %d",
                       sid->sid);
    }

    LOG("Play for stream sid %d, type %d, rsock %d, adapter %d, sock_id %d, "
        "rsock_id %d, handle %d",
        s->sid, sid->type, sid->rsock, sid->adapter, s->id, sid->rsock_id,
        s->sock);
    ad = get_adapter(sid->adapter);

    if (compare_tunning_parameters(
            sid->adapter,
            &sid->tp)) // close the adapter that is required to be closed
    {
    ...
    // TO DO: if a slave adapter changes the band, pol or diseqc, detach it from
    // the adapter and find a new one for it the same applies to a stream from a
    // master adapter where there are already slave adapters using it.

    // check if the adapter is not valid or if a slave SID is trying to change
    // frequency
    if (!ad ||
        (compare_tunning_parameters(sid->adapter, &sid->tp) &&
         ad->master_sid !=
             sid->sid)) // associate the adapter only at play (not at setup)
    {
                               ...
                               // sending empty packets to the dest
        {
            sid->do_play = 1;
            LOG_AND_RETURN(
                0, "Tune requested with no real parameters, ignoring ...");
        }
        if (ad) {
            LOG("slave stream tuning to a new frequency, finding a new "
                "adapter");
            close_adapter_for_stream(sid->sid, ad->id, 0);
        }
...
int decode_transport(sockets *s, char *arg, char *default_rtp, int start_rtp) {
    ...
    if (sid->type == 0) {
        ...
        if ((sid->rtcp_sock = sockets_add(sid->rtcp, &sa, sid->sid, TYPE_RTCP,
                                          (socket_action)rtcp_confirm, NULL,
                                          NULL)) < 0) // read rtcp
...
int enqueue_rtp_header(streams *sid, struct iovec *iov, int liov,
    ...
    timestamp =
        (uint32_t)(90000 *
                   ((ts.tv_sec * 1000000ll + ts.tv_nsec / 1000) / 1000000ll)) +
        (9 * ((ts.tv_sec * 1000000ll + ts.tv_nsec / 1000) % 1000000ll)) /
            100; // 90 kHz Clock
...
int check_new_transponder(adapter *ad, int rlen) {
    ...
    for (i = 0; i < rlen; i += 188) {
        ...
        if (b[0] == 0x47 && b[1] == 0x40 &&
            b[2] == 0) // pid 0, calculate transponder_id
        {
            tid = b[8] * 256 + b[9];
            if (tid != ad->wait_transponder_id) {
                ad->wait_transponder_id = tid;
                LOG("Got the new transponder %04X %d, position %d, %jd ms "
                    "after tune",
                    tid, tid, i, getTick() - ad->tune_time);
                memmove(ad->buf, ad->buf + i, rlen - i);
                return rlen - i;
            } else
                LOGM("Got old transponder id %04X %d, position %d, %jd ms "
                     "after tune",
                     tid, tid, i, getTick() - ad->tune_time);
        }
    }
    ...
    for (i = 0; i < ad->rlen; i += DVB_FRAME) {
        ...
        if (p) {
            ...
            if (p->count + 1 >= CRC_TS) {
                ...
                LOG("pid %d cnt %d count %d cc %d err %d crc %08X ", p->pid,
                    p->packets, p->count, p->cc, p->cc_err, p->crc);
                //			LOG("count %d", p->cnt);
                p->count = 0;
                p->cc_crc = 0;
            }
...
int process_packets_for_stream(streams *sid, adapter *ad) {
    ...
    LOGM("Processing done sid %d at pos %d, rtp header %d, rtp_pos %d, "
         "total_len "
         "%d, rlen %d",
         st_id, iiov, last_rtp_header, rtp_pos, total_len, ad->rlen);
...
int read_dmx(sockets *s) {
    ...
    if (ad->wait_new_stream &&
        (rtime - ad->tune_time < 200)) // check new transponder
    {
    ...
    if (c_time - bwtt > 1000) {
        ...
        if (bw > 2000 || bw_dmx > 2000) {
            ...
            LOG("BW %jd KB/s, DMX %jd KB/s, Buffered %.1f MB, Dropped: %.1f "
                "MB, ns/read %jd, r: %d, w: %d fw: %d, tt: %jd ms, memory %jd MB",
...
int rtcp_confirm(sockets *s) {
    ...
    if (!sid)
        return 0;
    LOG("%s: called for stream sid %d from %s:%d", __FUNCTION__, s->sid,
        get_stream_rhost(sid->sid, ra, sizeof(ra) - 1),
...
_symbols stream_sym[] = {
    {"st_enabled", VAR_AARRAY_INT8, st, 1, MAX_STREAMS,
     offsetof(streams, enabled)},
    {"st_play", VAR_AARRAY_INT, st, 1, MAX_STREAMS, offsetof(streams, do_play)},
    {"st_adapter", VAR_AARRAY_INT, st, 1, MAX_STREAMS,
     offsetof(streams, adapter)},
    {"st_useragent", VAR_AARRAY_STRING, st, 1, MAX_STREAMS,
     offsetof(streams, useragent)},
    ...
    {"st_overflow", VAR_FUNCTION_INT, (void *)&get_stream_overflow, 0,
     MAX_STREAMS, 0},
    {"st_buffered", VAR_FUNCTION_INT, (void *)&get_stream_buffered_size, 0,
     MAX_STREAMS, 0},
    {NULL, 0, NULL, 0, 0, 0}};
